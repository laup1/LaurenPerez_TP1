...
app.controller('statusCtrl', function ($scope, $http) {
    ...
    $scope.createStatus = function () {
        var req = {
            ...
            data: {'name': $scope.newName, 'status_id': $scope.createStatus.id}
        }
        $http(req)
                .success(function (response) {
                    //console.log(data.response.result);
                    // tell the user new subcategory was created
                    Materialize.toast('status successfully created', 4000);

                    // close modal
                    $('#modal-createStatus-form').modal('close');
                ...
                .error(function (response) {
                    //console.log(data.response.result);
                    // tell the user new subcategory was created
                    Materialize.toast('Could not create status', 4000);
                });
    }
    // read subcategories
    $scope.getAll = function () {
        ...
        $http(req)
                ...
                .error(function (response) {
                    ...
                    Materialize.toast('Could not retreive Status', 4000);
                })
                ;
        ...
        $http(req)
                .success(function (jsonData, status, headers, config) {
                    ...
                    $scope.createStatus = jsonData.data;
                })
                .error(function (response) {
                    Materialize.toast('Unable to retrieve Status.', 4000);
                });
        ...
        if (localStorage.getItem("token") === "no token") {
            ...
            $http(req)
                    .success(function (jsonData, status, headers, config) {
                        ...
                        $scope.selectedStatusId = jsonData.data.status_id;
                    })
                    .error(function (response) {
            ...
            $http(req)
                    .success(function (jsonData, status, headers, config) {
                        ...
                        // put the values in form
                        $scope.editStatus = jsonData.data;
                        angular.forEach($scope.editStatus, function (status) {
                            if (status.id == $scope.selectedStatusId) {
                                $scope.editStatus = status;
                            }
                        })
                        // show modal
                        $('#modal-editStatus-form').modal('open');
                    })
                    .error(function (response) {
                        Materialize.toast('Unable to retrieve Status.', 4000);
                    })
    ...
    $scope.updateStatus = function () {
        var req = {
            method: 'PUT',
            url: 'api/status/' + $scope.id,
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
            },
            data: {'name': $scope.actualName, 'status_id': $scope.editStatus.id}
        }
        $http(req)
                .success(function (response) {
                    // tell the user subcategory record was updated
                    Materialize.toast('Status successfully updated', 4000);
                    // close modal
                    $('#modal-editStatus-form').modal('close');
                ...
                .error(function (response) {
                    // tell the user subcategory record was not updated
                    //console.log(response);
                    Materialize.toast('Could not update Status', 4000);
            ...
            if (confirm("Are you sure?")) {
                ...
                $http(req)
                        .success(function (response) {
                            ...
                            Materialize.toast('status successfully deleted', 4000);
                        ...
                        .error(function (response) {
                            ...
                            Materialize.toast('Could not Status', 4000);
                        })
                        ;
            }
        }
